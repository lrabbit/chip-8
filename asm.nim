...
proc assembleOpcode(symbols: Table[string, uint16], opcode: uint16, vx = none(string), vy = none(string), value = none(string)): uint16 =
    var output = opcode

    if vx.isSome():
        output = output + parseOperand(vx.get(), pFirst)
    
    if vy.isSome():
        output = output + parseOperand(vy.get(), pSecond)

    if value.isSome():
        if isLiteral(value.get(), cast[uint8](value.get().len)):
            output = output + parseLiteral(value.get())
        else:
            if not symbols.contains(value.get()):
                panic(fmt"Symbol not found: {value}")
            
            output = output + symbols[value.get()]
    ...
    if operands.len == 2:
        if isRegister(operands[0]) == rtV:
            if isLiteral(operands[1], 2):
                return assembleOpcode(symbols, 0x7000, vx = some(operands[0]), value = some(operands[1]))
            elif isRegister(operands[1]) == rtV:
                return assembleOpcode(symbols, 0x8004, vx = some(operands[0]), vy = some(operands[1]))
        elif isRegister(operands[0]) == rtI and isRegister(operands[1]) == rtV:
            return assembleOpcode(symbols, 0xF01E, vx = some(operands[1]))
...
proc assembleAND(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV:
        return assembleOpcode(symbols, 0x8002, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleCALL(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isSymbol(symbols, operands[0]):
        return assembleOpcode(symbols, 0x2000, value = some(operands[0]))
...
proc assembleDRW(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 3 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV and isLiteral(operands[2], 1):
        return assembleOpcode(symbols, 0xD000, vx = some(operands[0]), vy = some(operands[1]), value = some(operands[2]))
...
proc assembleJP(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isSymbol(symbols, operands[0]):
        return assembleOpcode(symbols, 0x1000, value = some(operands[0]))
    elif operands.len == 2 and isRegister(operands[0]) == rtV and stripHead(operands[0]) == "0" and isSymbol(symbols, operands[1]):
        return assembleOpcode(symbols, 0xB000, value = some(operands[1]))
...
proc assembleLD(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2:
        if isRegister(operands[0]) == rtV:
            if isLiteral(operands[1], 2):
                return assembleOpcode(symbols, 0x6000, vx = some(operands[0]), value = some(operands[1]))
            elif isRegister(operands[1]) == rtV: 
                return assembleOpcode(symbols, 0x8000, vx = some(operands[0]), vy = some(operands[1]))
            elif isRegister(operands[1]) == rtDT: 
                return assembleOpcode(symbols, 0xF007, vx = some(operands[0]))
            elif isRegister(operands[1]) == rtK: 
                return assembleOpcode(symbols, 0xF00A, vx = some(operands[0]))
            elif isRegister(operands[1]) == rtArrI: 
                return assembleOpcode(symbols, 0xF065, vx = some(operands[0]))
            elif isRegister(operands[1]) == rtR: 
                return assembleOpcode(symbols, 0xF085, vx = some(operands[0]))
        elif isRegister(operands[1]) == rtV:
            if isRegister(operands[0]) == rtDT:
                return assembleOpcode(symbols, 0xF015, vx = some(operands[1]))
            elif isRegister(operands[0]) == rtST:
                return assembleOpcode(symbols, 0xF018, vx = some(operands[1]))
            elif isRegister(operands[0]) == rtF:
                return assembleOpcode(symbols, 0xF029, vx = some(operands[1]))
            elif isRegister(operands[0]) == rtB:
                return assembleOpcode(symbols, 0xF033, vx = some(operands[1]))
            elif isRegister(operands[0]) == rtArrI:
                return assembleOpcode(symbols, 0xF055, vx = some(operands[1]))
            elif isRegister(operands[0]) == rtHF:
                return assembleOpcode(symbols, 0xF030, vx = some(operands[1]))
            elif isRegister(operands[0]) == rtR:
                return assembleOpcode(symbols, 0xF075, vx = some(operands[1]))
        elif isRegister(operands[0]) == rtI and isSymbol(symbols, operands[1]):
            return assembleOpcode(symbols, 0xA000, value = some(operands[1]))
...
proc assembleOR(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV:
        return assembleOpcode(symbols, 0x8001, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleRND(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV and isLiteral(operands[1], 2):
        return assembleOpcode(symbols, 0xC000, vx = some(operands[0]), value = some(operands[1]))
...
proc assembleSE(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV:
        if isLiteral(operands[1], 2):
            return assembleOpcode(symbols, 0x3000, vx = some(operands[0]), value = some(operands[1]))
        elif isRegister(operands[1]) == rtV:
            return assembleOpcode(symbols, 0x5000, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleSHL(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isRegister(operands[0]) == rtV:
        return assembleOpcode(symbols, 0x800E, vx = some(operands[0]))
    elif operands.len == 2 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV:
        return assembleOpcode(symbols, 0x800E, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleSHR(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isRegister(operands[0]) == rtV:
        return assembleOpcode(symbols, 0x8006, vx = some(operands[0]))
    elif operands.len == 2 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV:
        return assembleOpcode(symbols, 0x8006, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleSKP(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isRegister(operands[0]) == rtV:
        return assembleOpcode(symbols, 0xE09E, vx = some(operands[0]))
...
proc assembleSKNP(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isRegister(operands[0]) == rtV:
        return assembleOpcode(symbols, 0xE0A1, vx = some(operands[0]))
...
proc assembleSNE(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV:
        if isLiteral(operands[1], 2):
            return assembleOpcode(symbols, 0x4000, vx = some(operands[0]), value = some(operands[1]))
        elif isRegister(operands[1]) == rtV:
            return assembleOpcode(symbols, 0x9000, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleSUB(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV:
        return assembleOpcode(symbols, 0x8005, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleSUBN(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV:
        return assembleOpcode(symbols, 0x8007, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleSYS(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isSymbol(symbols, operands[0]):
        return assembleOpcode(symbols, 0x0000, value = some(operands[0]))
...
proc assembleXOR(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 2 and isRegister(operands[0]) == rtV and isRegister(operands[1]) == rtV:
        return assembleOpcode(symbols, 0x8003, vx = some(operands[0]), vy = some(operands[1]))
...
proc assembleSCU(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isLiteral(operands[0], 1):
        return assembleOpcode(symbols, 0x00B0, value = some(operands[0]))
...
proc assembleSCD(symbols: Table[string, uint16], operands: seq[string]): uint16 = 
    ...
    if operands.len == 1 and isLiteral(operands[0], 1):
        return assembleOpcode(symbols, 0x00C0, value = some(operands[0]))
